...
BEGIN
	...
	BEGIN TRY
		...
		INTO #wt_UPDATE_WITH_APP_CATSHN_MST_101_1
		...
		WITH
			t1 AS (
				...
				INNER JOIN #wt_UPDATE_WITH_APP_CATSHN_MST_101_1 b
				...
				LEFT OUTER JOIN #wt_UPDATE_WITH_APP_CATSHN_MST_101 c
				ON a.SCSM_KCD = b.SCSM_KCD
					AND a.SCSM_HTCD = c.SCSM_HTCD
					AND a.SCSM_LCATCD = c.SCSM_LCATCD
					AND a.SCSM_MCATCD = c.SCSM_MCATCD
					AND a.SCSM_SCATCD = c.SCSM_SCATCD
					AND a.SCSM_SCD = c.SCSM_SCD
					AND a.SCSM_TEKIYOYMD = c.SCSM_TEKIYOYMD
				WHERE c.SCSM_KCD IS NULL
			)
		INSERT INTO #wt_UPDATE_WITH_APP_CATSHN_MST_101
		SELECT *, dbo.CONVERT_HTCD(SCSM_KCD, SCSM_HTCD) AS TENCD
		FROM t1;

		DROP TABLE #wt_UPDATE_WITH_APP_CATSHN_MST_101_1;
		...
		*/
		...
		WITH
			...
			t2 AS (
				...
				*/
				SELECT ACTM_HTCD, ACTM_LCATCD, ACTM_MCATCD, ACTM_CATORDER
				FROM APP_CAT_M
				WHERE ACTM_SCATCD = '0000'
					AND ACTM_JYOGAIFLG = 0
			),
			t3 AS (
		...
		SELECT KHTCD, LCAT, MCAT, SCAT, SKAK_SCD,
			...
			(LORDER * 100000) + (MORDER * 10000) + dbo.GET_MINKEISAIJYUN(KHTCD, SKAK_SCD) AS APPKEISAIJYUN,
