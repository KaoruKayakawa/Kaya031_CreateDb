...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GET_SHOHIN_M_INFO_ALLPERIOD_WEB]') AND type in (N'IF'))
DROP FUNCTION [dbo].[GET_SHOHIN_M_INFO_ALLPERIOD_WEB]
GO
...
-- =============================================
-- 名称			:GET_SHOHIN_M_INFO_ALLPERIOD
-- 機能			:全期間の受注商品マスタ（全店舗対応）取得
-- 引き数		: @paramHTCD	int			個店の販社店舗コード
-- 戻り値		: table
-- 作成日		: 2016/05/25  作成者 : MSYS M.Morishita
-- 修正日		: 2021/03/02　作成者 : 伊藤　掲載フラグ3対応追加
-- 修正日		: 2022/06/17　修正者 : 茅川　[SHM_BURUICD] 追加
-- =============================================
CREATE FUNCTION [dbo].[GET_SHOHIN_M_INFO_ALLPERIOD_WEB]
(	
...
(
	...
	select
		...
		T1.SHM_NEWSORTKEY,
		SHM_SIMETIME
		--2019/05/16 軽減税率TEST 山下 Add Start
		,SHM_TAXKBN
		--2019/05/16 軽減税率TEST 山下 Add End
		--2019/05/28 Msys 3次開発(注文確定_コメント機能追加-コメント表示区分追加) Add Start
		,SHM_COMMENTDISPFLG
		--2019/05/28 Msys 3次開発(注文確定_コメント機能追加-コメント表示区分追加) Add End
	from 
		(
