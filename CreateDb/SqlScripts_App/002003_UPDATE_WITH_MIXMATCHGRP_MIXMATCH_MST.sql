	...
	BEGIN TRY
		...
		WITH
			...
			t2 AS (
				...
				INNER JOIN t1
				ON a.SMGM_KCD = t1.SMGM_KCD
					AND a.SMGM_HTCD = t1.SMGM_HTCD
					AND a.SMGM_MMNO = t1.SMGM_MMNO
				LEFT OUTER JOIN #wt_UPDATE_WITH_MIXMATCHGRP_MST_101 b
				ON a.SMGM_KCD = b.SMGM_KCD
					AND a.SMGM_HTCD = b.SMGM_HTCD
					AND a.SMGM_MMNO = b.SMGM_MMNO
					AND a.SMGM_TEKIYOYMD = b.SMGM_TEKIYOYMD
				WHERE b.SMGM_KCD IS NULL
			)
		...
		WITH
			...
			t2 AS (
				...
				INNER JOIN t1
				ON a.SMIM_KCD = t1.SMIM_KCD
					AND a.SMIM_HTCD = t1.SMIM_HTCD
					AND a.SMIM_MMNO = t1.SMIM_MMNO
					AND a.SMIM_SCD = t1.SMIM_SCD
				LEFT OUTER JOIN #wt_UPDATE_WITH_MIXMATCH_MST_101 b
