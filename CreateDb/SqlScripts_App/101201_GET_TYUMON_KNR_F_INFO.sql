...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GET_TYUMON_KNR_F_INFO]') AND type in (N'IF'))
DROP FUNCTION [dbo].[GET_TYUMON_KNR_F_INFO]
GO
...
-- =============================================
CREATE FUNCTION [dbo].[GET_TYUMON_KNR_F_INFO]
(	
...
(
    ...
    from 
        (
            select 
				/*
				row_number() over(partition by TKF_SCD order by TKF_HTCD DESC , TKF_SCD , TKF_STR , TKF_INYMD) as RN ,
				*/
				row_number() over(partition by TKF_SCD order by TKF_HTCD DESC , TKF_STR , TKF_INYMD) as RN ,
