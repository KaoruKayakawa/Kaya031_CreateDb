...
BEGIN
	...
	BEGIN TRY
		...
		WITH
			t2 AS (
				SELECT DISTINCT a.STKF_KCD, a.STKF_HTCD, a.STKF_SCD, a.STKF_STR, a.STKF_END, a.STKF_TEKIYOYMD
				FROM TYUMON_KNR_MST_now a
		...
		INSERT INTO #wt_UPDATE_WITH_TYUMON_KNR_MST_101
		SELECT t1.*, dbo.CONVERT_HTCD(t1.STKF_KCD, t1.STKF_HTCD) AS TENCD
		FROM TYUMON_KNR_MST_now t1
		INNER JOIN t2
		ON t1.STKF_KCD = t2.STKF_KCD
			AND t1.STKF_HTCD = t2.STKF_HTCD
			AND t1.STKF_SCD = t2.STKF_SCD
			AND t1.STKF_STR = t2.STKF_STR
			AND t1.STKF_END = t2.STKF_END
