...
FROM (
	...
	WHERE TKF_DELFG = 0) d
LEFT OUTER JOIN (
	SELECT c.TKF_HTCD, c.TKF_SCD, c.TKF_STR, c.TKF_END, c.TKF_TEKIYOYMD, DATEADD(second, -1, MIN(c.TKF_TEKIYOYMD_END)) AS TKF_TEKIYOYMD_END
	FROM (
		SELECT a.TKF_HTCD, a.TKF_SCD, a.TKF_STR, a.TKF_END, a.TKF_TEKIYOYMD, b.TKF_TEKIYOYMD AS TKF_TEKIYOYMD_END
		FROM (
			SELECT *
			FROM TYUMON_KNR_F_now
			WHERE TKF_DELFG <> 1) a
	...
	GROUP BY c.TKF_HTCD, c.TKF_SCD, c.TKF_STR, c.TKF_END, c.TKF_TEKIYOYMD) e
ON d.TKF_HTCD = e.TKF_HTCD
	AND d.TKF_SCD = e.TKF_SCD
	AND d.TKF_STR = e.TKF_STR
	AND d.TKF_END = e.TKF_END
