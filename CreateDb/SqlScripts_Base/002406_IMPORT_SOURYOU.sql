...
-- [会社コード、店舗コード、商品コード、適用日、企画コード] で予約削除
		WITH
			t1 AS (
				SELECT CSO_KCD, CSO_TENCD, CSO_SCD, CSO_TEKIYOYMD, CSO_KIKAKUCD, CSO_IMPORTFILE
				FROM #wt_IMPORT_SOURYOU_1
				WHERE CSO_YDELKBN = 1
			),
			t2 AS (
				SELECT STKF_KCD, STKF_HTCD, STKF_SCD, STKF_TEKIYOYMD, STKF_KIKAKUCD
				FROM TYUMON_KNR_MST_now
				WHERE STKF_DELFG <> 1
			)
		SELECT t1.CSO_KCD, t1.CSO_TENCD, t1.CSO_SCD, t1.CSO_TEKIYOYMD, t1.CSO_KIKAKUCD, t1.CSO_IMPORTFILE, t2.STKF_KCD AS CSO_KCD_2
		...
		ON t1.CSO_KCD = t2.STKF_KCD
			...
			AND t1.CSO_KIKAKUCD = t2.STKF_KIKAKUCD;
  ...
  -- 削除対象レコードが存在しない削除設定を取得
		...
		SELECT t1.CSO_KCD, t1.CSO_TENCD, t1.CSO_SCD, t1.CSO_KIKAKUCD, t1.CSO_TEKIYOYMD,
...
-- [会社コード、店舗コード、商品コード、企画コード] でマスタ削除（※ [適用日] 時点）
		WITH
			t1 AS (
				SELECT CSO_KCD, CSO_TENCD, CSO_SCD, CSO_KIKAKUCD, CSO_TEKIYOYMD, CSO_IMPORTFILE
				FROM #wt_IMPORT_SOURYOU_1
				WHERE CSO_MDELKBN = 1
			)
		SELECT t1.CSO_KCD, t1.CSO_TENCD, t1.CSO_SCD, t1.CSO_KIKAKUCD, t1.CSO_TEKIYOYMD, t1.CSO_IMPORTFILE, t2.STKF_TEKIYOYMD
